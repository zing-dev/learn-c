//
// Created by zhangrongxiang on 2017/10/9 15:42
// File s2
//
...
int main(int argc, const char *argv[]) {
    ...
    //绑定socket：将创建的socket绑定到本地的IP地址和端口，此socket是半相关的，
    // 只是负责侦听客户端的连接请求，并不能用于和客户端通信
    ...
    //listen侦听 第一个参数是套接字，第二个参数为等待接受的连接的队列的大小，
    // 在connect请求过来的时候,完成三次握手后先将连接放到这个队列中，
    // 直到被accept处理。如果这个队列满了，且有新的连接的时候，对方可能会收到出错信息。
